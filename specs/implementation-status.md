...
- 2026-04-13: prepared local branch `codex/ci-truth`; first CI candidate commit is `6bb1930`, but a real GitHub Actions run is still unobserved because `origin` points to `https://github.com/USERNAME/REPO.git` and `git push` fails with `Repository not found`
